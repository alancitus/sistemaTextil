...
class ProveedorModel extends CI_Model
{
	public function Actualizar($data)
	{
		...
		if($data['Dni'] != '')
		{
			...
			else if ($this->db->query("SELECT COUNT(*) Total FROM proveedor WHERE Empresa_id = " . $this->user->Empresa_id . " AND id != $id AND Dni = '" . $data['Dni'] . "'")->row()->Total > 0)
			{
				$validacion = false;
	...
	public function Listar()
	{
		...
		$this->db->where($where);
		$this->jqgridmodel->Config($this->db->SELECT('COUNT(*) Total FROM proveedor')->get()->row()->Total);
