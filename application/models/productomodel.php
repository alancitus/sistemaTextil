...
class ProductoModel extends CI_Model
{
	...
	public function Actualizar($data)
	{
		...
		{
			$FConfig['file_name'] = $data['id'] . '_producto';
	...
	public function Registrar($data)
	{
		...
		if(isset($_FILES['Foto']))
		{
			$FConfig['file_name'] = $new_id . '_producto';
			$this->upload->initialize($FConfig);
			if ( ! $this->upload->do_upload('Foto') )
			{
				$this->responsemodel->message = $this->upload->display_errors();
			...
			{
				$upload = $this->upload->data();
				$this->db->where('id',$new_id)->update('producto', ['Foto'=>$upload['file_name']]);	
			}
		}
		
		$this->responsemodel->SetResponse(true);
		$this->responsemodel->href   = 'mantenimiento/producto/' . $new_id;
